...
class TestSymbolIslandGates:
    def test_passes_symbol_island_gate(self) -> None:
        train = {
            "total_return_pct": 3.0,
            "profit_factor": 1.2,
        ...
        val = {
            "total_return_pct": 2.0,
            "profit_factor": 1.1,
            "executed_trades": _cfg.PHASE3_SYMBOL_RULE_MIN_VAL_TRADES,
            "max_drawdown_pct": 4.0,
        }
        from gpu_fuzzy_trader.phases.phase2_support import (
            passes_migrant_target_gate,
            passes_symbol_island_robustness_gate,
        )

        assert passes_symbol_island_robustness_gate(train, val) is True
        assert passes_migrant_target_gate(train, val) is True

    def test_rejects_low_val_support(self) -> None:
        from gpu_fuzzy_trader.phases.phase2_support import (
            passes_symbol_island_robustness_gate,
        )
        ...
        assert passes_symbol_island_robustness_gate(train, val) is False
