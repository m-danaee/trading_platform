...
class TestSymbolSpecialistExport:
    def test_merges_identical_feature_rules_with_symbol_filters(self, monkeypatch):
        from gpu_fuzzy_trader.phases.phase3_rule_set import assemble_symbol_specialist_export

        monkeypatch.setattr(_cfg, "PHASE3_MIN_RULES", 1)
        monkeypatch.setattr(_cfg, "PHASE3_MAX_RULES", 5)
        monkeypatch.setattr(
            _cfg, "PHASE3_SYMBOL_RULE_MIN_TRAIN_TRADES", 1,
        )
        monkeypatch.setattr(
            _cfg, "PHASE3_SYMBOL_RULE_MIN_VAL_TRADES", 1,
        )

        shared_conditions = ["[feat_0] IS Very High", "[feat_1] IS Low"]
        symbol_pools = {
            "SYM_A": [{
                "conditions": list(shared_conditions),
                "chromosome": [0, 1],
                "objectives": {"sortino_ratio": 2.0, "total_return_pct": 3.0,
            ...
            "SYM_B": [{
                "conditions": list(shared_conditions),
                "chromosome": [0, 1],
                "objectives": {"sortino_ratio": 1.5, "total_return_pct": 2.0,
                               "max_drawdown_pct": 6.0, "win_rate": 52.0},
                "executed_trades": 25,
            }],
        }
        train_df = _make_df(n_rows=200, symbols=["SYM_A", "SYM_B"])
        val_df = _make_df(n_rows=100, symbols=["SYM_A", "SYM_B"])

        with patch(
            "gpu_fuzzy_trader.phases.phase3_rule_set._pool_entry_passes_symbol_validation",
            return_value=(True, {"total_return_pct": 2.0, "executed_trades": 20,
                               "profit_factor": 1.2, "max_drawdown_pct": 5.0},
                          {"total_return_pct": 1.0, "executed_trades": 10,
                           "profit_factor": 1.1, "max_drawdown_pct": 4.0}),
        ):
            result = assemble_symbol_specialist_export(
                "long", symbol_pools, train_df, val_df,
            )

        assert result.get("symbol_specialist") is True
        assert len(result["rules_set"]) == 1
        conds = result["rules_set"][0]["conditions"]
        assert conds[0].startswith("symbol is ")
        assert "[feat_0] IS Very High" in conds

    def test_skips_symbols_without_validated_rules(self, monkeypatch):
        from gpu_fuzzy_trader.phases.phase3_rule_set import assemble_symbol_specialist_export
        ...
        train_df = _make_df(n_rows=100, symbols=["SYM_A", "SYM_B"])
        val_df = _make_df(n_rows=50, symbols=["SYM_A", "SYM_B"])

        with patch(
            "gpu_fuzzy_trader.phases.phase3_rule_set._pool_entry_passes_symbol_validation",
            side_effect=[
                (True, {"total_return_pct": 2.0}, {"total_return_pct": 1.0}),
            ],
        ):
            result = assemble_symbol_specialist_export(
                "long", symbol_pools, train_df, val_df,
        ...
        assert "SYM_B" in result.get("skipped_symbols", [])
