...
## Corpus Check
- 185 files · ~259,092 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4787 nodes · 10804 edges · 177 communities (166 shown, 11 thin omitted)
...
Cohesion: 0.04
Nodes (93): _assign_eval_result(), _behavior_descriptor(), _binary_tournament_pick(), _build_diversity_reference(), _constraint_violations(), _count_deployable_preview(), _das_dennis(), _deduplicate_selection_indices() (+85 more)

### Community 1 - ".run"
Cohesion: 0.09
Nodes (17): _condition_feature_names(), _filter_compatible_previous_pool(), _filter_pool_by_admission(), _pool_path_key(), Extract feature names from textual conditions like: "[feat] IS Value". Returns…, Keep only previous pool entries compatible with current feature selection.…, Build monthly windows from unsampled val and apply the admission gate. Never…, Run the evolutionary search and return the Pareto-front pool. Also persists… (+9 more)

### Community 2 - "rb_governor.py"
Cohesion: 0.06
Nodes (70): expectancy_lcb_pct(), Return a conservative lower bound for per-trade net expectancy. Exact CPU…, _available_symbols(), _balanced_phase2_shortlist(), _candidate_positive_symbols(), _diversification_beam(), _diversification_shortlist(), _eval_cv_fold_returns() (+62 more)

### Community 3 - "_make_engine"
Cohesion: 0.11
Nodes (16): _make_df(), _make_engine(), All TP trades → win_rate = 100%., No losing trades → profit_factor = 99.0., Build a minimal DataFrame for testing., position_notional = equity * capital_pct/100 * leverage when exposure is free., position_notional is capped when exposure is nearly full., When exposure is full, position_notional = 0. (+8 more)

### Community 4 - "_should_plateau_early_stop_phase2"
Cohesion: 0.03
Nodes (69): _make_offspring_population(), Shared val-cadence check for both NSGA-II fallback and NSGA-III loops. Val…, Generate pop_size offspring via binary tournament, crossover, mutation.…, Run Phase 2 NSGA-III evolution. Returns (pareto_pool, history) or state., run_phase2_evolution(), _should_plateau_early_stop_phase2(), _should_run_val_this_gen(), Evolutionary algorithm drivers for Phase 2. (+61 more)

### Community 5 - "test_evox_runner.py"
Cohesion: 0.05
Nodes (45): _evaluate_population_indices(), _inherit_val_metrics_from_global_cache(), _inject_diversity_recovery(), Phase2EvolutionState, Replace a fraction of the population with fresh or archive-mutated seeds., Evaluate unevaluated individuals, preferring batch simulate_rule_batch., Copy val_* from global cache for identical chromosomes when val is skipped.…, Resumable NSGA-III state for symbol-island epoch scheduling. (+37 more)
...
Cohesion: 0.05
Nodes (57): CPUBacktestEngine, CPU backtest engine that exactly mirrors evaluator_v5.ipynb's…, _combined_return_score(), _cost_stress_gate(), _evaluate_ruleset(), evaluate_strategy_governor(), _optimize_risk(), _profit_amp_evaluate_candidate() (+49 more)

### Community 8 - "validate_config"
Cohesion: 0.11
...
Cohesion: 0.13
Nodes (38): _random_active_class(), canonicalize_slots(), _clamp_slot_gene(), compute_rule_signals_numpy(), count_active_slots(), crossover_sparse(), dense_to_sparse(), empty_slots() (+30 more)

### Community 11 - "_plateau_diversity_restart"
Cohesion: 0.24
...
Cohesion: 0.04
Nodes (68): FileHandler, count_trials(), Estimate the number of adaptive evaluations represented by artifacts., _context_coverage_for_direction(), _context_coverage_preflight(), _context_coverage_report(), _log_phase_entry(), _log_pipeline_config() (+60 more)
...
### Community 14 - "run_phase2_evolution_epoch"
Cohesion: 0.11
Nodes (18): extract_deployable_migrants(), StageLabel, Return elite deployable-preview entries suitable for guarded migration., Evolve one island epoch and return updated resumable state., run_phase2_evolution_epoch(), Evolve this island for one scheduler epoch., _FakeEngine, Verify reset_plateau clears restart counters on resumed state. (+10 more)
...
Cohesion: 0.05
Nodes (63): downcast_numeric_df(), df_slim.py — Reduce DataFrame memory for backtest engines. Keeps only columns…, Downcast label and feature columns to float32; symbol to category (inplace)., data/loader.py — Data_Loader Stateless CSV loading with full preparation…, _chronological_half_split(), _holdout_embargo_split(), _purged_walk_forward_split(), DataFrame (+55 more)
...
Cohesion: 0.08
Nodes (40): filter_df_to_symbols(), phase2_history_path(), phase2_pool_path(), DataFrame, Return rows for the given symbols; raises if column missing or no rows., Resolve Phase 2 pool path., Resolve Phase 2 history path., clear_global_metrics_cache() (+32 more)
...
Cohesion: 0.13
Nodes (19): _condition_feature(), Exception, Path, writer.py — Output_Writer Serializes RuleSet dicts to JSON with exact schema…, Validate an optional symbol filter (symbol is X / [symbol] IS X)., Validate a single condition string. Accepts either: - [feature_name] IS Fuzzy…, Validate a single rule object. Returns the validated rule dict (with numeric…, Validate and normalise a rule_set dict. Applies all schema constraints… (+11 more)

### Community 36 - "config.py"
Cohesion: 0.07
Nodes (34): _config_check(), ConfigError, effective_min_trade_support(), effective_sortino_min_trade_threshold(), effective_val_trade_floor_for_objectives(), _finite_config_number(), get_purged_wf_reference_rows(), get_seed() (+26 more)
...
Cohesion: 0.05
Nodes (60): context_contract(), context_contract_digest(), Return the full context contract for strategy/dataset identity., Return a stable hash of the static contract and fitted enrichment., Any, Stable provenance identifiers for Phase 2 feature rules. The RB Governor may…, Hash the complete economic strategy, including exit policy. ``phase2_rule_id``…, strategy_id() (+52 more)
...
Cohesion: 0.05
Nodes (45): required_barrier_columns(), Data_Splitter, load_cached_split_if_fresh(), Load cached split parquets when they are newer than the source CSV. Validates…, Chronological train/validation splitter., Module-level wrapper around ``Data_Splitter.split_and_persist``., split_and_persist(), _make_df() (+37 more)
...
Cohesion: 0.08
Nodes (19): _batch_eval_rule_set_pickled(), _expectancy_lcb_pct(), _expected_shortfall_pct(), Simulate a rule set on rows [row_start, row_end) without copying the df. Used…, Simulate a rule set and return performance metrics. Parameters ----------…, Simulate using a precomputed rule-evaluation mask cache. The mandatory…, Drop cached entries on rows the fixed context mask forbids. ``idx`` validity is…, Evaluate multiple rule sets in parallel (ProcessPool, thread fallback). (+11 more)

### Community 60 - "ValueError"
Cohesion: 0.07
Nodes (33): _append_allocated_entries(), _build_rule_signal_mask(), compute_entry_time_priority(), _normalize_direction(), _parse_condition(), precompute_release_indices(), precompute_release_indices_from_offsets(), DataFrame (+25 more)

### Community 61 - "resolve_phase2_stage_params"
Cohesion: 0.07
Nodes (25): _diversity_recovery_min_unique_ratio(), True when Stage A viability is critically low and search has plateaued., _should_inject_diversity_recovery(), _should_viability_recovery(), island_stage_budgets(), IslandStagePlan, StageLabel, Resolved stage and remaining generation budget for one symbol island. (+17 more)

### Community 62 - "Data_Loader"
Cohesion: 0.05
Nodes (37): Data_Loader, Stateless data loader for the GPU-Fuzzy Trading Pipeline., Build a flat list of per-symbol metric dicts for CSV output. Uses the…, _isolate_phase5_reporter_outputs(), _make_df(), _make_rule_set(), DataFrame, fixture (+29 more)

### Community 63 - "TestWriteSpearmanCorrelationReport"
Cohesion: 0.13
...
Cohesion: 0.21
Nodes (11): context_permission_column(), context_trigger_column(), Return the direction-specific permission column name., Return the direction-specific LWC pullback-reversal trigger column., _context_feature_direction(), Return the strategy direction a context column belongs to (if any)., _coverage(), _print_split_coverage() (+3 more)
...
Cohesion: 0.08
Nodes (20): Compute a non-annualized Sortino Ratio from per-trade returns., _safe_profit_factor(), _sortino_ratio_from_returns(), JointPortfolioEngine, DataFrame, Joint long/short portfolio simulation. Phase 2 and RB score each direction…, Evaluate long and short rule books in one net-position account., Backward-compatible wrapper returning penalty only. (+12 more)

### Community 73 - "run_rb_governor_pipeline"
Cohesion: 0.08
Nodes (35): effective_rb_min_distinct_symbols(), Return the RB coverage target for the active debug universe. Full runs keep…, feature_conditions_only(), phase2_rule_id(), Return normalized non-symbol, non-context conditions in order. Symbol scope and…, Hash the immutable Phase 2 feature logic and its discovery scope., _assert_capital_budget(), _assert_mandatory_context() (+27 more)
...
### Community 78 - "_print_run_summary"
Cohesion: 0.33
Nodes (5): _phase5_test_metrics(), _print_run_summary(), Any, Return test-split metrics (supports nested Phase 5 result shape)., Print a concise CLI summary for a full or single-phase run.
...
### Community 80 - ".load_dataset"
Cohesion: 0.19
Nodes (10): _ensure_labels(), load_dataset(), DataFrame, Keep supplied labels or derive all labels from raw OHLCV columns. The original…, Load a CSV dataset with full preparation pipeline: 1. Read CSV with comma…, Module-level wrapper around ``Data_Loader.load_dataset``., Validate the mandatory trend-context contract on an enriched frame. Fails…, validate_context_columns() (+2 more)

### Community 81 - ".save_archive"
Cohesion: 0.22
...
Cohesion: 0.10
Nodes (34): HealthCheck, prop_settings(), Shared Hypothesis settings for property tests. Set PYTEST_LOW_MEMORY=1 to scale…, Hypothesis settings with optional low-memory example scaling., _assert_parity(), _make_engines(), _make_parity_df(), parity_scenario_strategy() (+26 more)
...
### Community 90 - "slim_backtest_df"
Cohesion: 0.21
Nodes (12): prune_train_columns(), DataFrame, Return a copy with only meta, label, and requested feature columns. Parameters…, Drop unused feature columns from the full training split after Phase 1., slim_backtest_df(), Build train/val backtest engines., DataFrame, Unit tests for backtest DataFrame slimming. (+4 more)
...
Cohesion: 0.03
Nodes (81): effective_min_trade_pool_floor(), effective_pool_min_val_trades(), IslandHyperparams, Resolved Phase 2 knobs for cluster or orphan slices., _count_pop_viable(), Population-wide count passing the pool trade floor., _pool_entry_passes_admission(), Check stored train/val metrics on a pool JSON entry. (+73 more)
...
### Community 103 - "_derive_val_sample_seed"
Cohesion: 0.19
...
### Community 113 - "test_rb_fail_closed.py"
Cohesion: 0.29
...
Cohesion: 0.06
Nodes (43): effective_monthly_min_trades(), _monthly_selection_certificate(), _profit_amp_monthly_summary(), Require a composed team to be mostly non-loss across calendar windows., Evaluate a ruleset on validation-only chronological windows for the amplifier…, build_monthly_windows(), _datetime_series(), evaluate_rule_set_monthly() (+35 more)
...
Cohesion: 0.11
Nodes (16): _make_walk_forward_fold_engines(), Split val_selection into n_splits chronological folds + optional tail holdout.…, _make_synthetic_df(), DataFrame, 2 splits + tail holdout → 2 fold engines + 1 tail engine., tail_holdout_frac=0 → tail engine is None., Each symbol's data is divided into contiguous chunks across folds., Single symbol without symbol column is handled gracefully. (+8 more)
...
### Community 133 - "_chromosome_with_min_active"
Cohesion: 0.22
Nodes (7): _chromosome_with_min_active(), _pop_contains_dense_seed(), ndarray, f3 profit_factor branch mirrors win_rate train+val blend. Covers three…, Check whether *pop* contains a sparse row matching dense *seed*., Build a dense chromosome with exactly MIN_CONDITIONS active genes., TestEvaluateChromosome
...
Cohesion: 0.05
Nodes (38): compute_phase2_objectives_from_metrics(), _diversity_penalty_blended(), _phenotype_bucket_key(), Discretise objective-relevant metrics for behavioral diversity., Hamming OR phenotype-bucket crowding penalty (same weight on both)., Build Phase 2 minimisation objectives from precomputed train/val metrics.…, tanh-saturated Sortino so the best-front member moves with progress. The…, _saturating_sortino() (+30 more)
...
### Community 139 - "_validate_pool_schema"
Cohesion: 0.36
...
### Community 146 - "apply_fuzzy_feature_scaling"
Cohesion: 0.29
...
### Community 151 - "_apply_colab_gpu_defaults"
Cohesion: 0.33
...
### Community 153 - "TestNClustersDefined"
Cohesion: 0.33
...
Cohesion: 0.06
Nodes (24): _crowding_distance(), _dominates(), _hamming_distance(), _non_dominated_sort(), _pareto_sortino_stats(), Return True if solution *a* dominates *b* (all objectives ≤, at least one <)., NSGA-II non-dominated sorting. Parameters ---------- objectives : np.ndarray…, Compute crowding distance for solutions in *front*. Parameters ----------… (+16 more)
...
Cohesion: 0.08
Nodes (47): _archive_direction(), _archive_objective_vector(), attach_cv_fold_returns_batch(), _build_cpu_archive_engine(), _build_pool_from_archive(), _chromosome_batch(), _chromosome_for_pool_export(), _crossover() (+39 more)
...
### Community 160 - "_legacy_writer_contract"
Cohesion: 0.67
...
- **Why does `CPUBacktestEngine` connect `CPUBacktestEngine` to `rb_governor.py`, `_make_walk_forward_fold_engines`, `_make_engine`, `_make_df`, `Rule_Pool_Generator`, `_apply_monthly_admission_gate`, `TestContextEntryPaths`, `test_cpu_engine_properties.py`, `test_phase2_use_gpu_flag.py`, `run_pipeline.py`, `CandidateRecord`, `phase2_island_scheduler.py`, `_Phase5JSONEncoder`, `phase2_rule_pool.py`, `ValidationError`, `research_integrity.py`, `gpu_engine.py`, `test_jax_compat.py`, `_compute_rule_signal_mask`, `_build_entries_from_rule_set`, `GPUBacktestEngine`, `._simulate_rule_set_entries`, `ValueError`, `_symbol_specialized_variants`, `_apply_dynamic_rule`, `test_cpu_engine.py`, `run_rb_governor_pipeline`, `TestGPUCPUNumericalParity`, `test_gpu_engine.py`, `filter_migrants_for_cluster`, `prop_settings`, `slim_backtest_df`, `test_output_writer_properties.py`, `test_trend_context.py`, `test_phase2_gpu_throughput.py`, `baselines.py`, `_jax_compute_trade_outcomes`, `_jax_compute_rule_signals`, `test_rb_governor_tail_holdout.py`, `MonthlyWindowSummary`, `._engine`, `OOS_Evaluator`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `Reporter` connect `Reporter` to `.run`, `TestEquityCurveDateAxis`, `TestPlotDistributionAndEquity`, `TestWriteStrategyEvaluationTable`, `._ensure_dir`, `Rule_Pool_Generator`, `TestPlotPerRuleBreakdown`, `TestWriteSpearmanCorrelationReport`, `DataFrame`, `prop_settings`, `run_pipeline.py`, `test_reporter.py`, `_Phase5JSONEncoder`, `phase2_rule_pool.py`, `OOS_Evaluator`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `Rule_Pool_Generator` connect `Rule_Pool_Generator` to `.run`, `log_memory_rss`, `_derive_epoch_seed`, `_chromosome_with_min_active`, `TestRulePoolGeneratorRun`, `CPUBacktestEngine`, `TestEvictClusterSignatures`, `compute_phase2_objectives_from_metrics`, `_validate_pool_schema`, `Pipeline_Orchestrator`, `run_phase2_evolution_epoch`, `_apply_monthly_admission_gate`, `TestRefreshObjectivesOnResumeGate`, `_make_train_df`, `test_phase2_use_gpu_flag.py`, `run_pipeline.py`, `Reporter`, `TestNClustersDefined`, `test_phase2_rule_pool.py`, `TestF3PathResolution`, `phase2_island_scheduler.py`, `phase2_rule_pool.py`, `TestDeferredWarmup`, `test_phase2_window_rotation.py`, `TestPoolAdmissionOverfitRatioGate`, `_get_dont_cares`, `test_phase2_rule_pool_properties.py`, `test_phase2_island_scheduler.py`, `test_plateau_state_leak.py`, `.save_archive`, `filter_migrants_for_cluster`, `test_crash_fix_and_run_logging.py`, `TestRunLogHandlerLifecycle`, `slim_backtest_df`, `TestEndToEndRotation`, `TestSeedDirectionUniqueness`, `_derive_val_sample_seed`, `TestValLeakGate`?**
