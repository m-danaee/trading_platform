...
## Corpus Check
- 185 files · ~258,970 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4786 nodes · 10801 edges · 172 communities (162 shown, 10 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 550 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cf4f943a`
...
6. `Pipeline_Orchestrator` - 63 edges
7. `_run_nsga3()` - 58 edges
8. `Feature_Selector` - 58 edges
9. `Data_Loader` - 57 edges
10. `_run_nsga2_fallback()` - 57 edges
...
Cohesion: 0.04
Nodes (99): _assign_eval_result(), _behavior_descriptor(), _build_diversity_reference(), _constraint_violations(), _count_deployable_preview(), _das_dennis(), _deduplicate_selection_indices(), _empty_eval_stats() (+91 more)

### Community 1 - ".run"
Cohesion: 0.07
Nodes (35): _archive_feature_signature(), _archive_objective_vector(), _condition_feature_names(), _deployable_archive_pool_entries(), _filter_compatible_previous_pool(), _filter_pool_by_admission(), _is_better_archive_entry(), _merge_archive_entries() (+27 more)

### Community 2 - "CPUBacktestEngine"
Cohesion: 0.06
Nodes (80): CPUBacktestEngine, CPU backtest engine that exactly mirrors evaluator_v5.ipynb's…, expectancy_lcb_pct(), Return a conservative lower bound for per-trade net expectancy. Exact CPU…, _balanced_phase2_shortlist(), _candidate_positive_symbols(), _cost_stress_gate(), _diversification_beam() (+72 more)
...
Cohesion: 0.06
Nodes (38): StageLabel, Evolve one island epoch and return updated resumable state., Run Phase 2 NSGA-III evolution. Returns (pareto_pool, history) or state., run_phase2_evolution(), run_phase2_evolution_epoch(), Evolutionary algorithm drivers for Phase 2., _FakeEngine, Verify reset_plateau clears restart counters on resumed state. (+30 more)

### Community 5 - "test_evox_runner.py"
Cohesion: 0.06
Nodes (34): extract_deployable_migrants(), _inherit_val_metrics_from_global_cache(), _inject_diversity_recovery(), Phase2EvolutionState, Replace a fraction of the population with fresh or archive-mutated seeds., Copy val_* from global cache for identical chromosomes when val is skipped.…, Return elite deployable-preview entries suitable for guarded migration., Resumable NSGA-III state for symbol-island epoch scheduling. (+26 more)
...
Cohesion: 0.09
Nodes (55): encoder.py — Encoder Maps gene integer values to fuzzy value names, formats…, _random_active_class(), _chromosome_for_pool_export(), _crossover(), _init_population(), _pool_seed_chromosomes(), phase2_rule_pool.py — Rule_Pool_Generator (Phase 2) GPU-accelerated multi-…, Initialise a population of chromosomes. *init_strategy* ``"stratified_sparse"``… (+47 more)

### Community 11 - "test_phase2_plateau_restart.py"
Cohesion: 0.07
Nodes (22): TestRunPhase2EvolutionFallback, FakeEngine, Unit tests for diversity restart on first plateau (Fix D). Verifies that: -…, Reinitialised slots have objectives=np.inf and metrics_cache={}., Even with large Pareto front, at most 10 elite are preserved., Verify AC3 (mutation boost applied to offspring) and AC4 (streak reset). Runs…, Mutation boost is applied to offspring after restart, then reverts;…, Plateau trigger works normally (unchanged). (+14 more)

### Community 12 - ".run"
Cohesion: 0.07
Nodes (32): FileHandler, __main__.py — Entry point for `python -m gpu_fuzzy_trader.run_pipeline` Allows…, _log_phase_entry(), _log_pipeline_config(), main(), _now_iso(), DataFrame, Attach a FileHandler to the root logger writing to RUN_LOG_PATH. Returns the… (+24 more)

### Community 13 - "Rule_Pool_Generator"
Cohesion: 0.06
Nodes (30): Return a copy with only meta, label, and requested feature columns. Parameters…, slim_backtest_df(), _derive_val_sample_seed(), Derive a deterministic validation sample seed from the training seed. This…, Phase 2: GPU-accelerated multi-objective evolutionary rule pool generation.…, Inject guarded migration seeds for the next epoch., Build train/val backtest engines., Restore slimmed training data from cache (no re-sampling needed). (+22 more)

### Community 14 - "Pipeline_Orchestrator"
Cohesion: 0.11
Nodes (25): _context_coverage_preflight(), Pipeline_Orchestrator, Temporarily rebind all cached output paths for one pipeline run., Load Phase 2 pools for both directions from the persistent cache., Ensure at least one valid strategy exists before standalone Phase 5., Log split-aware context coverage and fail before Phase 2 if unusable., Top-level orchestrator for the GPU-Fuzzy Trading Pipeline. Runs all five phases…, Return the active output root for this run. (+17 more)
...
Cohesion: 0.06
Nodes (33): _downsample_chronological(), _evaluate_rule_on_window(), _largest_safe_range(), _monthly_admission_source_df(), DataFrame, Evaluate a single pool rule on a single monthly window. Returns the full window…, Prefer unsampled monthly val; fall back to sampled slim val., Preserve per-symbol time order required by exposure/release simulation. (+25 more)
...
Cohesion: 0.06
Nodes (57): _chronological_half_split(), _holdout_embargo_split(), _purged_walk_forward_split(), DataFrame, data/splitter.py — Data_Splitter Per-symbol chronological split for Phase 2,…, Per-symbol chronological first or second half of *df*. ``purge_rows`` is…, Split validation into purged fitness and selection halves per symbol. The gap…, Return whether cached internal halves match the purged geometry. (+49 more)
...
Cohesion: 0.17
Nodes (28): _candidate_coverage_symbols(), CandidateRecord, _compose_ruleset(), Symbol coverage for compose diversity. Prefer **traded** symbols from backtest…, patch, _cand(), engines(), _generalist_cand() (+20 more)

### Community 28 - "phase2_island_scheduler.py"
Cohesion: 0.07
Nodes (42): filter_df_to_symbols(), phase2_history_path(), phase2_pool_path(), DataFrame, Return rows for the given symbols; raises if column missing or no rows., Resolve Phase 2 pool path., Resolve Phase 2 history path., clear_global_metrics_cache() (+34 more)

### Community 29 - "_preserve_deployable_elites"
Cohesion: 0.08
Nodes (29): _binary_tournament_pick(), _build_rank_and_crowding(), environmental_selection_nsga2(), _make_offspring_population(), _preserve_deployable_elites(), Generate pop_size offspring via binary tournament, crossover, mutation.…, Per-individual Pareto rank (lower is better) and crowding distance., Pick the better of two random candidates (lower rank, higher crowding). (+21 more)
...
Cohesion: 0.12
Nodes (20): _condition_feature(), Exception, Path, writer.py — Output_Writer Serializes RuleSet dicts to JSON with exact schema…, Validate an optional symbol filter (symbol is X / [symbol] IS X)., Validate a single condition string. Accepts either: - [feature_name] IS Fuzzy…, Validate a single rule object. Returns the validated rule dict (with numeric…, Validate and normalise a rule_set dict. Applies all schema constraints… (+12 more)

### Community 36 - "config.py"
Cohesion: 0.05
Nodes (43): _apply_colab_gpu_defaults(), context_contract(), context_contract_digest(), effective_min_trade_support(), effective_sortino_min_trade_threshold(), effective_val_trade_floor_for_objectives(), get_purged_wf_reference_rows(), holdout_train_val_label() (+35 more)
...
Cohesion: 0.14
Nodes (27): Any, Hash the complete economic strategy, including exit policy. ``phase2_rule_id``…, strategy_id(), _canonical_json(), count_trials(), dataset_manifest(), ExperimentLedger, forward_acceptance_lock_path() (+19 more)
...
Cohesion: 0.06
Nodes (38): required_barrier_columns(), Data_Splitter, load_cached_split_if_fresh(), Load cached split parquets when they are newer than the source CSV. Validates…, Chronological train/validation splitter., Module-level wrapper around ``Data_Splitter.split_and_persist``., split_and_persist(), _make_df() (+30 more)
...
### Community 48 - "validate_config"
Cohesion: 0.05
Nodes (62): _config_check(), ConfigError, _debug_symbol_universe_size(), effective_config_snapshot(), effective_min_profitable_symbols(), _finite_config_number(), Active symbol count when debug scope is on; None for full-universe runs., Cap cross-symbol profitability gate to the active universe size. With… (+54 more)

### Community 49 - "_get_dont_cares"
Cohesion: 0.08
Nodes (21): _count_active_conditions(), _get_dont_cares(), _mutate(), Mutate a chromosome (returns a copy). When activating a dont_care gene, feature…, Return array of dont_care sentinels for each feature., Count active rule conditions (sparse slots or dense dont_care encoding)., Tests for H5/M4/M5 evolution convergence behaviors. Covers: - HoF trimming at…, C5 mutation bias: force symbol-gene to dont_care / inactive with probability… (+13 more)
...
Cohesion: 0.10
Nodes (19): compute_cluster_generation_budgets(), Resolve per-island generation budgets. By default each island receives the full…, parametrize, Unit tests for cluster island scheduler budget math and epoch guard., Multi-symbol clusters each get the full island budget (not split)., The dead migration helper _should_migrate_this_round has been deleted. Verify…, AC: n_clusters is assigned inside _run_cluster_islands so the migration guard…, n_clusters must be assigned in _run_cluster_islands for the migration guard at… (+11 more)
...
Cohesion: 0.06
Nodes (39): _append_allocated_entries(), _build_rule_signal_mask(), compute_entry_time_priority(), _expectancy_lcb_pct(), _expected_shortfall_pct(), _parse_condition(), precompute_release_indices(), precompute_release_indices_from_offsets() (+31 more)

### Community 61 - "resolve_phase2_stage_params"
Cohesion: 0.09
Nodes (17): _diversity_recovery_min_unique_ratio(), True when Stage A viability is critically low and search has plateaued., _should_inject_diversity_recovery(), _should_viability_recovery(), StageLabel, Return stage-tuned hyperparameters. When *stage* is None (single-stage Phase…, resolve_phase2_stage_params(), Tiny deployable archive must not IndexError on viability recovery. (+9 more)

### Community 62 - "test_phase5_oos.py"
Cohesion: 0.05
Nodes (34): Build a flat list of per-symbol metric dicts for CSV output. Uses the…, _isolate_phase5_reporter_outputs(), _make_df(), _make_rule_set(), DataFrame, fixture, Unit tests for gpu_fuzzy_trader.phases.phase5_oos.OOS_Evaluator Tests cover: -…, Verify plot_equity_curve is called for all three splits and handles empty logs. (+26 more)
...
### Community 65 - "context_permission_column"
Cohesion: 0.16
Nodes (15): context_permission_column(), context_trigger_column(), Return the direction-specific permission column name., Return the direction-specific LWC pullback-reversal trigger column., _context_feature_direction(), Return the strategy direction a context column belongs to (if any)., _context_coverage_for_direction(), _context_coverage_report() (+7 more)
...
Cohesion: 0.10
Nodes (29): _attach_source_symbol_filters(), _available_symbols(), _ensure_symbol_filtered_rule(), _has_symbol_condition(), _is_recency_good(), _is_symbol_condition(), Add deterministic single-condition RB candidates. Evolution is deliberately…, Island/cluster symbols carried on Phase 2 pool entries. (+21 more)
...
Cohesion: 0.08
Nodes (20): _normalize_direction(), Compute a non-annualized Sortino Ratio from per-trade returns., _safe_profit_factor(), _sortino_ratio_from_returns(), JointPortfolioEngine, DataFrame, Evaluate long and short rule books in one net-position account., normalize_symbol_value() (+12 more)

### Community 73 - "run_rb_governor_pipeline"
Cohesion: 0.08
Nodes (41): effective_rb_min_distinct_symbols(), mandatory_context_conditions(), Return the RB coverage target for the active debug universe. Full runs keep…, Return the fixed, mandatory context conditions for *direction*., feature_conditions_only(), phase2_rule_id(), Stable provenance identifiers for Phase 2 feature rules. The RB Governor may…, Return normalized non-symbol, non-context conditions in order. Symbol scope and… (+33 more)
...
Cohesion: 0.13
Nodes (11): Any, Typed, versioned profile for the active research contract., Small stable surface for comparing experiments. The legacy config module…, ResearchProfile, _phase5_test_metrics(), _print_run_summary(), Any, Append one auditable record after a pipeline evaluation completes. (+3 more)
...
### Community 80 - "phase5_oos.py"
Cohesion: 0.07
Nodes (36): downcast_numeric_df(), prune_train_columns(), DataFrame, df_slim.py — Reduce DataFrame memory for backtest engines. Keeps only columns…, Downcast label and feature columns to float32; symbol to category (inplace)., Drop unused feature columns from the full training split after Phase 1., Joint long/short portfolio simulation. Phase 2 and RB score each direction…, _ensure_labels() (+28 more)

### Community 81 - "TestSplitRatio"
Cohesion: 0.21
...
Cohesion: 0.11
Nodes (30): HealthCheck, prop_settings(), Shared Hypothesis settings for property tests. Set PYTEST_LOW_MEMORY=1 to scale…, Hypothesis settings with optional low-memory example scaling., _assert_parity(), _make_engines(), _make_parity_df(), DataFrame (+22 more)
...
### Community 90 - "_normalize_for_association"
Cohesion: 0.21
...
Cohesion: 0.08
Nodes (12): _enriched(), Regression: symbols arrive interleaved (sorted by datetime then symbol), so…, Higher-timeframe state is only published after that bar completes and aligned…, _raw_tape(), TestCausalPublicationTiming, TestContextContract, TestContextIdentity, TestInputAndBoundaryContracts (+4 more)
...
### Community 103 - "island_stage_budgets"
Cohesion: 0.29
...
### Community 111 - "TestSparsePositiveMode"
Cohesion: 0.22
...
Cohesion: 0.21
Nodes (13): Build the per-direction symbol certificate policy. Specialist islands are…, _symbol_gate_policy(), _BatchEngine, _candidate(), _metrics(), _pool_entry(), Focused regressions for certificate-first RB and Phase 2 selection., test_certificate_rejects_eth_only_and_accepts_balanced_team() (+5 more)

### Community 122 - "MonthlyWindowSummary"
Cohesion: 0.06
Nodes (41): effective_monthly_min_trades(), _monthly_selection_certificate(), Require a composed team to be mostly non-loss across calendar windows., build_monthly_windows(), _datetime_series(), evaluate_rule_set_monthly(), monthly_penalty(), monthly_return_counts_as_good() (+33 more)
...
### Community 127 - "Data_Loader"
Cohesion: 0.08
Nodes (21): Data_Loader, Stateless data loader for the GPU-Fuzzy Trading Pipeline., OOS_Evaluator, DataFrame, Out-of-sample evaluator for Phase 5. Loads the final long/short strategies,…, Run out-of-sample evaluation. Parameters ---------- allowed_directions :…, Prepare test data using Data_Loader.load_dataset(). Applies the same…, Load prepared train, validation, and test datasets. (+13 more)
...
### Community 134 - "test_phase5_oos_properties.py"
Cohesion: 0.19
...
### Community 138 - "TestZeroRatioBoundary"
Cohesion: 0.22
...
### Community 141 - "TestSparseSignedMode"
Cohesion: 0.33
...
Cohesion: 0.10
Nodes (27): _archive_direction(), attach_cv_fold_returns_batch(), _build_cpu_archive_engine(), _build_pool_from_archive(), _chromosome_batch(), CvFoldValEvaluator, _entry_validation_per_symbol_metrics(), _pool_entry_rank() (+19 more)
...
- **Why does `CPUBacktestEngine` connect `CPUBacktestEngine` to `TestMakeWalkForwardFoldEngines`, `_make_engine`, `_score_metrics`, `_make_df`, `phase2_rule_pool.py`, `Rule_Pool_Generator`, `TestContextEntryPaths`, `_make_train_df`, `test_cpu_engine_properties.py`, `test_phase2_use_gpu_flag.py`, `CandidateRecord`, `phase2_island_scheduler.py`, `_build_pool_from_archive`, `ValidationError`, `gpu_engine.py`, `test_jax_compat.py`, `_compute_rule_signal_mask`, `_build_entries_from_rule_set`, `TestEvalCvFoldReturns`, `GPUBacktestEngine`, `get_normalized_symbol_array`, `ValueError`, `nested_walk_forward.py`, `_symbol_specialized_variants`, `barrier.py`, `_apply_dynamic_rule`, `test_cpu_engine.py`, `run_rb_governor_pipeline`, `TestGPUCPUNumericalParity`, `test_gpu_engine.py`, `phase5_oos.py`, `filter_migrants_for_cluster`, `prop_settings`, `test_trend_context.py`, `test_phase2_gpu_throughput.py`, `baselines.py`, `_jax_compute_trade_outcomes`, `_jax_compute_rule_signals`, `test_rb_governor_tail_holdout.py`, `test_certificate_first_selection.py`, `MonthlyWindowSummary`, `._engine`, `Data_Loader`?**
  _High betweenness centrality (0.133) - this node is a cross-community bridge._
- **Why does `Reporter` connect `Reporter` to `.run`, `config.py`, `TestEquityCurveDateAxis`, `TestPlotDistributionAndEquity`, `phase2_rule_pool.py`, `TestWriteStrategyEvaluationTable`, `._ensure_dir`, `Rule_Pool_Generator`, `phase5_oos.py`, `TestPlotPerRuleBreakdown`, `DataFrame`, `prop_settings`, `test_reporter.py`, `_build_pool_from_archive`, `Data_Loader`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `Rule_Pool_Generator` connect `Rule_Pool_Generator` to `.run`, `CPUBacktestEngine`, `log_memory_rss`, `_derive_epoch_seed`, `TestInitPopulation`, `TestRulePoolGeneratorRun`, `TestEvictClusterSignatures`, `compute_phase2_objectives_from_metrics`, `phase2_rule_pool.py`, `.load_pool`, `.run`, `Pipeline_Orchestrator`, `_apply_monthly_admission_gate`, `TestRefreshObjectivesOnResumeGate`, `_make_train_df`, `test_phase2_use_gpu_flag.py`, `run_pipeline.py`, `Reporter`, `test_phase2_rule_pool.py`, `phase2_island_scheduler.py`, `TestDeferredWarmup`, `test_phase2_window_rotation.py`, `passes_pool_admission_gate`, `_get_dont_cares`, `test_phase2_rule_pool_properties.py`, `test_phase2_island_scheduler.py`, `test_plateau_state_leak.py`, `run_rb_governor_pipeline`, `filter_migrants_for_cluster`, `test_crash_fix_and_run_logging.py`, `TestRunLogHandlerLifecycle`, `TestEndToEndRotation`, `TestSeedDirectionUniqueness`, `_MockGenerator`, `TestValLeakGate`?**
