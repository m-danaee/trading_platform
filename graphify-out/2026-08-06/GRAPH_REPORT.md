...
## Corpus Check
- 185 files · ~258,008 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4766 nodes · 10754 edges · 190 communities (180 shown, 10 thin omitted)
...
### Community 56 - ".simulate_rule_set"
Cohesion: 0.22
...
Cohesion: 0.06
Nodes (35): attach_barrier_outcomes(), barrier_column_names(), configured_barrier_pairs(), _first_touch_for_symbol(), _first_touch_for_symbol_numba(), _first_touch_for_symbol_python(), _number_token(), DataFrame (+27 more)
...
Cohesion: 0.12
Nodes (17): _append_allocated_entries(), _parse_condition(), precompute_release_indices(), cpu_engine.py — CPUBacktestEngine Exact Python/NumPy replication of…, Parse '[feature_name] IS Fuzzy Value Name' → (feature_name, value_name)., Sort entries for v5 capital allocation (timestamp, rule, symbol, row)., For each row, find the row index where symbol_bar_index + max_hold_candles is…, Simulate a rule set on rows [row_start, row_end) without copying the df. Used… (+9 more)
...
- **Why does `CPUBacktestEngine` connect `CPUBacktestEngine` to `TestMakeWalkForwardFoldEngines`, `_make_engine`, `_score_metrics`, `_make_df`, `Rule_Pool_Generator`, `test_cpu_engine_properties.py`, `test_phase2_use_gpu_flag.py`, `CandidateRecord`, `phase2_island_scheduler.py`, `phase2_rule_pool.py`, `trade_support_penalty`, `TestMakeFoldEnginesTailHoldout`, `ValidationError`, `test_phase2_batch_evaluator_parity.py`, `gpu_engine.py`, `test_jax_compat.py`, `_compute_rule_signal_mask`, `.__init__`, `.simulate_rule_set`, `TestEvalCvFoldReturns`, `GPUBacktestEngine`, `ValueError`, `nested_walk_forward.py`, `_symbol_specialized_variants`, `cpu_engine.py`, `_apply_dynamic_rule`, `test_cpu_engine.py`, `TestGPUCPUNumericalParity`, `test_gpu_engine.py`, `Data_Loader`, `filter_migrants_for_cluster`, `prop_settings`, `test_trend_context.py`, `baselines.py`, `_jax_compute_trade_outcomes`, `_jax_compute_rule_signals`, `test_rb_governor_tail_holdout.py`, `MonthlyWindowSummary`, `._engine`, `OOS_Evaluator`?**
