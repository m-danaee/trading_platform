...
## Corpus Check
- 192 files · ~282,135 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4882 nodes · 11305 edges · 177 communities (167 shown, 10 thin omitted)
...
Cohesion: 0.03
Nodes (130): _assign_eval_result(), _behavior_descriptor(), _binary_tournament_pick(), _build_diversity_reference(), _build_rank_and_crowding(), clear_global_metrics_cache(), _constraint_violations(), _count_deployable_preview() (+122 more)

### Community 1 - "test_evox_runner.py"
Cohesion: 0.04
Nodes (52): _diversity_recovery_min_unique_ratio(), _inherit_val_metrics_from_global_cache(), Phase2EvolutionState, Copy val_* from global cache for identical chromosomes when val is skipped.…, Resumable NSGA-III state for symbol-island epoch scheduling., Resolve the plateau patience value based on stage params or config., Earliest gen for plateau stop., _resolve_plateau_min_generation() (+44 more)

### Community 2 - "gpu_engine.py"
Cohesion: 0.13
Nodes (21): attach_barrier_outcomes(), barrier_column_names(), configured_barrier_pairs(), _first_touch_for_symbol(), _first_touch_for_symbol_numba(), _first_touch_for_symbol_python(), _number_token(), DataFrame (+13 more)

### Community 3 - "_make_engine"
Cohesion: 0.09
Nodes (19): _make_df(), _make_engine(), Trades with tiny equity should be skipped., All TP trades → win_rate = 100%., No losing trades → profit_factor = 99.0., Build a minimal DataFrame for testing., position_notional = equity * capital_pct/100 * leverage when exposure is free., position_notional is capped when exposure is nearly full. (+11 more)

### Community 4 - "Pipeline_Orchestrator"
Cohesion: 0.06
Nodes (55): FileHandler, _dataframe_schema_sha256(), _dataframe_sha256(), _git_commit_id(), _log_phase_entry(), _log_pipeline_config(), _merge_mtf_lwc_runtime_columns(), _now_iso() (+47 more)
...
### Community 8 - "_make_rule_set"
Cohesion: 0.19
Nodes (3): _make_rule_set(), TestLoadAndValidateHappyPath, TestWriteHappyPath
...
Cohesion: 0.12
Nodes (12): The fallback must not switch f3 from CV return to PF after gen 0., TestRunPhase2EvolutionFallback, FakeEngine, Verify AC3 (mutation boost applied to offspring) and AC4 (streak reset). Runs…, Mutation boost is applied to offspring after restart, then reverts;…, Minimal engine stub that returns fixed metrics., Verify the plateau branch decision logic in _run_nsga3. We monkeypatch…, First plateau triggers _plateau_diversity_restart. (+4 more)
...
### Community 18 - "_split"
Cohesion: 0.08
Nodes (19): DataFrame, Helper: build df, patch paths, run split, return (train, val)., Compute expected train/val per symbol under holdout+embargo split., floor(N * HOLDOUT_TRAIN_FRACTION) rows go to train., Remaining rows after embargo gap go to validation., For N=101: floor(101 * HOLDOUT_TRAIN_FRACTION) used, not round., train + validation + embargo dropped == total rows., train + val + embargo_dropped == total for each symbol. (+11 more)
...
Cohesion: 0.10
Nodes (37): apply_purge_embargo(), build_master_temporal_folds(), export_fold_boundaries(), _format_fold_predictions(), generate_oof_scores(), _get_datetime_series(), Any, DataFrame (+29 more)
...
Cohesion: 0.14
Nodes (29): _candidate_coverage_symbols(), _candidate_positive_symbols(), CandidateRecord, _compose_ruleset(), Return supported positive validation symbols for one candidate., Symbol coverage for compose diversity. Prefer **traded** symbols from backtest…, _cand(), engines() (+21 more)

### Community 28 - "Rule_Pool_Generator"
Cohesion: 0.06
Nodes (24): _derive_val_sample_seed(), Derive a deterministic validation sample seed from the training seed. This…, Phase 2: GPU-accelerated multi-objective evolutionary rule pool generation.…, Restore slimmed training data from cache (no re-sampling needed)., Rebuild engines after ``park_engines`` dropped GPU state., Rule_Pool_Generator, _make_feature_infos(), In holdout mode, val engine must be built for pool admission even when… (+16 more)
...
Cohesion: 0.12
Nodes (11): Output_Writer, Serializes RuleSet dicts to JSON and loads/validates existing JSON files.…, parametrize, Unit tests for gpu_fuzzy_trader.output.writer — Output_Writer Tests cover: -…, Verify the exact example from the spec works end-to-end., TestLoadAndValidateErrors, TestSpecExample, TestValidationErrorType (+3 more)

### Community 36 - "config.py"
Cohesion: 0.06
Nodes (37): _config_check(), ConfigError, context_contract(), effective_min_trade_support(), effective_monthly_min_trades(), effective_phase2_val_return_floor_pct(), effective_val_trade_floor_for_objectives(), filter_df_to_symbols() (+29 more)
...
Cohesion: 0.09
Nodes (27): _archive_direction(), attach_cv_fold_returns_batch(), _build_cpu_archive_engine(), _build_pool_from_archive(), _chromosome_batch(), _chromosome_for_pool_export(), _crowding_distance(), CvFoldValEvaluator (+19 more)
...
Cohesion: 0.09
Nodes (27): _batch_metrics_from_array(), _build_event_batch(), _exposure_slot_capacity(), _fast_reject_result_rows(), _jax_compute_rule_signals_sparse_batch(), _jax_open_slot(), _jax_release_open_slots(), _jax_simulate_equity_batch() (+19 more)
...
Cohesion: 0.08
Nodes (34): Any, ndarray, Evaluate hierarchical soft-veto composition on input arrays., Evaluate this frozen candidate on raw OHLCV rows. No thresholds, weights,…, Convert candidate to a JSON-serializable dictionary., Compute deterministic strategy SHA-256 identifier., compose_bidirectional_signals(), compose_hierarchical_signals() (+26 more)

### Community 46 - "test_certificate_first_selection.py"
Cohesion: 0.10
Nodes (29): _diversification_beam(), _passes_symbol_concentration_gate(), _passes_symbol_contribution_certificate(), _passes_tail_selection_gate(), _portfolio_selection_certificate(), Any, Require positive, supported validation PnL from multiple symbols. Symbol…, Build the per-direction symbol certificate policy. Specialist islands are… (+21 more)

### Community 47 - "TestBuildMonthlyWindows"
Cohesion: 0.20
...
### Community 51 - "test_evolution_convergence.py"
Cohesion: 0.19
...
### Community 56 - "test_phase2_rule_pool.py"
Cohesion: 0.04
Nodes (97): assign_strata_to_indices(), build_feature_sampling_probs(), pick_active_count(), _pick_active_index(), _pick_inactive_index(), ndarray, _random_active_class(), phase2_init.py — Sparsity-guided stratified population initialization for Phase… (+89 more)

### Community 57 - "_make_rule"
Cohesion: 0.14
Nodes (8): _make_rule(), Spot-check a variety of valid fuzzy value names., Write rule_set to a temp file and reload the raw JSON., TestWriteConditionValidation, TestWriteFieldCoercion, TestWriteSymbolConditionValidation, TestWriteTruncation, _write_and_reload()
...
### Community 59 - "CPUBacktestEngine"
Cohesion: 0.05
Nodes (45): _append_allocated_entries(), _build_rule_signal_mask(), compute_entry_time_priority(), CPUBacktestEngine, _expectancy_lcb_pct(), _expected_shortfall_pct(), precompute_release_indices(), precompute_release_indices_from_offsets() (+37 more)
...
Cohesion: 0.13
Nodes (11): Build the selected Phase 2 backend for the sampled train frame., Return whether this generator should avoid allocating a JAX engine.…, Attach optional floor overrides; safe when *owner* is a partial mock., Build an engine on *df* using the same backend selection logic., _minimal_backtest_df(), DataFrame, MonkeyPatch, Phase 2 respects PHASE2_USE_GPU=False without loading JAX GPU engine. (+3 more)
...
### Community 63 - "run_pipeline.py"
Cohesion: 0.06
Nodes (36): apply_fuzzy_feature_scaling(), fit_fuzzy_feature_scaling(), Any, DataFrame, Train-fitted normalization for ordinal ``ff_*`` fuzzy feature codes. The…, Build a train-only scaling contract for ordinal ``ff_*`` columns., Apply an existing train-fitted scaling contract in place and return *df*., __main__.py — Entry point for `python -m gpu_fuzzy_trader.run_pipeline` Allows… (+28 more)
...
### Community 69 - "runtime.py"
Cohesion: 0.13
Nodes (30): attach_frozen_layer_scores(), attach_oof_layer_scores(), _causal_score_columns(), ensemble_layer_scores(), evaluate_candidate_frame(), _or_rule_masks(), _prefix_features(), prepare_causal_mtf_frame() (+22 more)
...
Cohesion: 0.08
Nodes (20): _normalize_direction(), Compute a non-annualized Sortino Ratio from per-trade returns., _safe_profit_factor(), _sortino_ratio_from_returns(), JointPortfolioEngine, DataFrame, Evaluate long and short rule books in one net-position account., Backward-compatible wrapper returning penalty only. (+12 more)
...
### Community 86 - ".run"
Cohesion: 0.11
Nodes (11): DataFrame, Return an explicit, non-success result for a failed split., Save a split report, marking consumed test data as diagnostic-only., Save the combined per-symbol performance CSV., Run out-of-sample evaluation. Parameters ---------- allowed_directions :…, Load prepared train, validation, and test datasets., Load and validate a strictly newer, untouched forward period., Load selected features for a direction when available. (+3 more)
...
### Community 90 - "test_phase2_val_sim_interval.py"
Cohesion: 0.15
...
Cohesion: 0.08
Nodes (28): _largest_safe_range(), DataFrame, Build train/val backtest engines., Return (start, end) of the largest contiguous bar range not in *forbidden*. The…, Cap *total_rows* so the per-symbol request fits within the safe range. When…, Resolve Phase 2 row budget then sample with aligned symbol windows., _resolve_sample_total_rows(), sample_df_for_phase2() (+20 more)
...
Cohesion: 0.03
Nodes (45): compute_phase2_objectives_from_metrics(), _diversity_penalty_blended(), _hamming_distance(), _phenotype_bucket_key(), Hamming distance between two chromosomes (active pairs when sparse)., Discretise objective-relevant metrics for behavioral diversity., Hamming OR phenotype-bucket crowding penalty (same weight on both)., Penalty for weak cross-symbol robustness on one split. (+37 more)
...
Cohesion: 0.06
Nodes (62): compute_archive_hash(), compute_rule_hash(), get_default_archive_path(), load_mtf_archive_payload(), load_mtf_rule_archive(), normalize_timeframe(), Any, Path (+54 more)
...
Cohesion: 0.23
Nodes (19): _passes_pool_admission_impl(), _positive_good_reject_reasons(), Human-readable reasons why ``_is_positive_good`` failed (diagnostics)., _finite_number(), gate_positive_good(), _metric_int(), positive_good_reject_reasons(), PositiveGoodThresholds (+11 more)
...
### Community 104 - "test_data_splitter_properties.py"
Cohesion: 0.18
...
Cohesion: 0.11
Nodes (32): context_contract_digest(), Return a stable hash of the static contract and fitted enrichment., Any, Stable provenance identifiers for Phase 2 feature rules. The RB Governor may…, Hash the complete economic strategy, including exit policy. ``phase2_rule_id``…, strategy_id(), _canonical_json(), count_trials() (+24 more)
...
### Community 113 - "compute_rule_weights"
Cohesion: 0.17
Nodes (14): compute_ensemble_direction_and_strength(), compute_rule_weights(), deduplicate_rules(), Any, ndarray, Decoupled Direction & Strength Ensembling for Hierarchical MTF. Calculates…, Compute non-negative rule weights from validation edge and stability scores.…, Deduplicate rules by condition set and direction prior to ensembling. If… (+6 more)
...
### Community 116 - "cpu_engine.py"
Cohesion: 0.05
Nodes (50): _batch_eval_rule_set_pickled(), _jax_runtime_loaded(), cpu_engine.py — CPUBacktestEngine Exact Python/NumPy replication of…, Simulate a rule set and return performance metrics. Parameters ----------…, Evaluate multiple rule sets without forking an active JAX runtime., Top-level worker for ProcessPoolExecutor (must be picklable)., Return whether forking would inherit an already multithreaded JAX runtime., _rule_symbols_for_allocation() (+42 more)
...
Cohesion: 0.08
Nodes (32): build_monthly_windows(), _datetime_series(), evaluate_rule_set_monthly(), monthly_penalty(), monthly_return_counts_as_good(), MonthlyWindowSummary, DataFrame, Series (+24 more)
...
### Community 126 - "test_evaluator_clean_writer.py"
Cohesion: 0.18
...
### Community 127 - "._prune_splits_after_phase1"
Cohesion: 0.25
...
### Community 132 - ".prepare_test_data"
Cohesion: 0.36
...
### Community 133 - "test_phase2_offspring_batch.py"
Cohesion: 0.29
...
### Community 135 - "TestChronologicalOrdering"
Cohesion: 0.29
...
### Community 139 - "rb_governor.py"
Cohesion: 0.07
Nodes (60): expectancy_lcb_pct(), Return a conservative lower bound for per-trade net expectancy. Exact CPU…, feature_conditions_only(), phase2_rule_id(), Return normalized non-symbol, non-context conditions in order. Symbol scope and…, Hash the immutable Phase 2 feature logic and its discovery scope., _available_symbols(), _balanced_phase2_shortlist() (+52 more)

### Community 140 - "TestPerSymbolIndependence"
Cohesion: 0.40
...
### Community 142 - "phase2_rule_pool.py"
Cohesion: 0.03
Nodes (96): effective_min_trade_pool_floor(), effective_pool_min_val_trades(), IslandHyperparams, Optional Phase 2 floor overrides (tests and diagnostics)., extract_deployable_migrants(), StageLabel, Return elite deployable-preview entries suitable for guarded migration., Evolve one island epoch and return updated resumable state. (+88 more)

### Community 143 - "ValueError"
Cohesion: 0.07
Nodes (43): _align_upstream_scores(), _build_layer_frame(), _candidate_key(), _directional_pareto_front(), discover_directional_layer(), _eligible_numeric_features(), _fit_fold_candidates(), _frame_hash() (+35 more)
...
### Community 151 - "_isolate_phase2_archive_paths"
Cohesion: 0.67
...
Cohesion: 0.09
Nodes (21): _monthly_admission_source_df(), _pool_path_key(), Any, Return the sidecar that binds a reusable pool to its inputs., Hash an artifact without loading it all into RAM., Prefer unsampled monthly val; fall back to sampled slim val., Build monthly windows from unsampled val and apply the admission gate. Never…, Return the full current-run context required for pool admission. (+13 more)
...
Cohesion: 0.09
Nodes (38): mandatory_context_conditions(), Return the fixed, mandatory context conditions for *direction*., _assert_capital_budget(), _assert_mandatory_context(), _enforce_capital_budget(), _passes_tail_holdout_gate(), Path, PathLike (+30 more)
...
- **Why does `CPUBacktestEngine` connect `CPUBacktestEngine` to `gpu_engine.py`, `_make_engine`, `Pipeline_Orchestrator`, `_score_metrics`, `_make_df`, `rb_governor.py`, `phase2_rule_pool.py`, `_apply_monthly_admission_gate`, `ValueError`, `test_cpu_engine_properties.py`, `CandidateRecord`, `Rule_Pool_Generator`, `_build_pool_from_archive`, `run_rb_governor_pipeline`, `_compute_rule_signal_mask`, `test_certificate_first_selection.py`, `test_jax_compat.py`, `TestEvalCvFoldReturns`, `nested_walk_forward.py`, `._build_engine_for_df`, `run_pipeline.py`, `_apply_dynamic_rule`, `test_cpu_engine.py`, `test_gpu_engine.py`, `_symbol_specialized_variants`, `_build_entries_from_rule_set`, `.run`, `test_gpu_engine_properties.py`, `GPUBacktestEngine`, `test_phase2_window_rotation.py`, `mtf/__init__.py`, `baselines.py`, `_jax_compute_trade_outcomes`, `OOS_Evaluator`, `cpu_engine.py`, `_jax_compute_rule_signals`, `test_rb_governor_tail_holdout.py`, `MonthlyWindowSummary`, `._engine`?**
  _High betweenness centrality (0.173) - this node is a cross-community bridge._
- **Why does `Reporter` connect `Reporter` to `DataFrame`, `TestEquityCurveDateAxis`, `_build_pool_from_archive`, `TestPlotDistributionAndEquity`, `TestWriteSpearmanCorrelationReport`, `._ensure_dir`, `TestWriteStrategyEvaluationTable`, `OOS_Evaluator`, `phase2_rule_pool.py`, `prop_settings`, `TestPlotPerRuleBreakdown`, `cpu_engine.py`, `.run`, `test_reporter.py`, `Rule_Pool_Generator`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `Rule_Pool_Generator` connect `Rule_Pool_Generator` to `log_memory_rss`, `Pipeline_Orchestrator`, `TestValLeakGate`, `TestRulePoolGeneratorRun`, `_pareto_sortino_stats`, `rb_governor.py`, `phase2_rule_pool.py`, `ValueError`, `_apply_monthly_admission_gate`, `TestRefreshObjectivesOnResumeGate`, `_make_train_df`, `Reporter`, `.finalize_island`, `_build_pool_from_archive`, `TestEndToEndRotation`, `prop_settings`, `test_crash_fix_and_run_logging.py`, `_dominates`, `test_phase2_rule_pool.py`, `CPUBacktestEngine`, `._build_engine_for_df`, `run_pipeline.py`, `test_plateau_state_leak.py`, `TestRunLogHandlerLifecycle`, `.save_archive`, `test_phase2_window_rotation.py`, `compute_phase2_objectives_from_metrics`, `_derive_epoch_seed`, `_validate_pool_schema`, `passes_pool_admission_gate`?**
