...
from gpu_fuzzy_trader.phases.phase3_objectives import (
    ...
    feature_conditions_key,
    has_duplicate_rules as _has_duplicate_rules,
    normalize_export_symbol,
...
def _scoped_symbol_df(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
...
def _rule_deployability_score(train_metrics: dict, val_metrics: dict) -> float:
    from gpu_fuzzy_trader.phases.phase2_support import deployability_rank_score

    return deployability_rank_score(train_metrics, val_metrics)


def _pool_entry_passes_symbol_validation(
    entry: dict,
    symbol: str,
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    ...
    """Return (passes, train_metrics, val_metrics) for one symbol-scoped rule."""
    from gpu_fuzzy_trader.phases.phase2_support import passes_symbol_island_robustness_gate
    ...
    try:
        scoped_train = _scoped_symbol_df(train_df, symbol)
        scoped_val = _scoped_symbol_df(val_df, symbol)
    except ValueError:
        return False, {}, {}

    train_engine = CPUBacktestEngine(
        scoped_train, feature_modes, direction,
    )
    val_engine = CPUBacktestEngine(
        scoped_val, feature_modes, direction,
    )
    train_metrics = train_engine.simulate_rule_set([rule])
    val_metrics = val_engine.simulate_rule_set([rule])
    passes = passes_symbol_island_robustness_gate(train_metrics, val_metrics)
    return passes, train_metrics, val_metrics


def _pick_best_symbol_pool_entry(
    pool: list[dict],
    symbol: str,
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    ...
    best_score = -np.inf
    for entry in pool:
        passes, train_m, val_m = _pool_entry_passes_symbol_validation(
            entry, symbol, train_df, val_df, direction, feature_modes,
        )
        if not passes:
            continue
        score = _rule_deployability_score(train_m, val_m)
        if score > best_score:
            best_score = score
            best_entry = entry
    return best_entry
...
def assemble_symbol_specialist_export(
    direction: str,
    ...
    """
    Select validated per-symbol rules and emit a flat export with symbol filters.

    Identical feature-condition sets across symbols merge into one rule whose
    ``conditions`` begin with one or more ``symbol is X`` clauses.
    """
    feature_names: set[str] = set()
    for pool in symbol_pools.values():
        for entry in pool:
            for cond in entry.get("conditions", []):
                if isinstance(cond, str) and cond.startswith("["):
                    name = cond.split("] IS ", 1)[0][1:].strip()
                    if name:
                        feature_names.add(name)
    feature_modes = {
        name: "positive" for name in feature_names
    }

    merged: dict[tuple[str, ...], list[tuple[str, dict]]] = {}
    skipped_symbols: list[str] = []
    symbol_assignments: dict[str, int] = {}

    for symbol in sorted(symbol_pools.keys(), key=lambda s: str(s)):
        pool = symbol_pools.get(symbol) or []
        if not pool:
            skipped_symbols.append(str(symbol))
            continue
        best = _pick_best_symbol_pool_entry(
            pool, symbol, train_df, val_df, direction, feature_modes,
        )
        if best is None:
            skipped_symbols.append(str(symbol))
            continue
        feat_key = feature_conditions_key(best.get("conditions", []))
        merged.setdefault(feat_key, []).append((str(symbol), best))

    export_rules: list[dict] = []
    for feat_key, sym_entries in merged.items():
        sym_entries.sort(key=lambda row: normalize_export_symbol(row[0]))
        symbol_labels = [
            normalize_export_symbol(sym) for sym, _ in sym_entries
        ]
        template = sym_entries[0][1]
        conditions = [f"symbol is {label}" for label in symbol_labels]
        conditions.extend(feat_key)
        export_rules.append({
            "tp": float(template.get("tp", _cfg.PHASE2_TP)),
            "sl": float(template.get("sl", _cfg.PHASE2_SL)),
            "capital_pct": float(template.get("capital_pct", _cfg.PHASE2_CAPITAL_PCT)),
            "conditions": conditions,
        })
        for idx, (sym, _) in enumerate(sym_entries, start=1):
            symbol_assignments[normalize_export_symbol(sym)] = len(export_rules)

    export_rules.sort(
        key=lambda rule: feature_conditions_key(rule.get("conditions", [])),
    )
    max_rules = min(int(_cfg.PHASE3_MAX_RULES), 5)
    export_rules = export_rules[:max_rules]

    if len(export_rules) < int(_cfg.PHASE3_MIN_RULES):
        reason = (
            f"symbol-specialist export has {len(export_rules)} deployable "
            f"merged rules (skipped_symbols={skipped_symbols})"
        )
        rejected = _build_rejected_output_dict(direction, reason)
        rejected["symbol_specialist"] = True
        rejected["skipped_symbols"] = skipped_symbols
        rejected["symbol_assignments"] = symbol_assignments
        return rejected

    output = _build_output_dict(export_rules, direction)
    output["symbol_specialist"] = True
    output["skipped_symbols"] = skipped_symbols
    output["symbol_assignments"] = symbol_assignments
    return output


def run_symbol_specialist_export(
    direction: str,
    symbol_pools: dict[str, list[dict]],
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
) -> dict:
    """Assemble and persist the flat symbol-filtered export for one direction."""
    result = assemble_symbol_specialist_export(
        direction, symbol_pools, train_df, val_df,
    )
    out_path = _OUTPUT_PATHS[direction]
    ...
    logger.info(
        "Phase 3 [%s] symbol-specialist export: %d rules, skipped_symbols=%s → %s",
        direction,
        len(result.get("rules_set", [])),
        result.get("skipped_symbols", []),
        out_path,
    )
    return result
