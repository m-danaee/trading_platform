...
def _run_nsga2_fallback(
    ...
    try:
        from gpu_fuzzy_trader import config as _cfg
        from gpu_fuzzy_trader.phases.phase2_rule_pool import _build_fallback_pool_from_population
        min_pool = int(getattr(_cfg, "PHASE2_FALLBACK_MIN_POOL_SIZE", 0))
        if getattr(_cfg, "PHASE2_FALLBACK_ENABLED", False) and len(pareto_pool) < min_pool:
            fallback = _build_fallback_pool_from_population(
                population, feature_infos, dont_cares, engine, metrics_by_chrom=metrics_by_chrom
            )
            pareto_pool = pareto_pool + fallback
            logger.info("%s: fallback pool added %d final-population rules", tag, len(fallback))
    except Exception as exc:
        logger.warning("%s: fallback pool construction failed: %s", tag, exc)
