...
def _scalar_score(
    ...
    """Higher is better.  Dominant term: validation return / max drawdown."""
    ...
    penalty = max(0.0, float(objectives[1]) - val_dd)
    ratio = return_to_drawdown(val_ret, val_dd, float(getattr(_cfg, "RETURN_DD_FLOOR", 1.0)))
    score = (
        20.0 * ratio
        + 1.20 * val_ret
        ...
        - 0.20 * val_dd
        - 1.15 * penalty
        - max(0, int(getattr(_cfg, "AUTO_SEARCH_SCORE_MIN_TRADES", 80)) - val_trades) * 0.035
    )
    return float(score)
...
def greedy_rule_set_search(
    ...
    for k in range(2, max_rules + 1):
        ...
        if round_best is not None:
            min_improve = float(getattr(_cfg, "PHASE3_GREEDY_MIN_IMPROVEMENT", 0.0))
            if (
                bool(getattr(_cfg, "PHASE3_GREEDY_EARLY_STOP", True))
                and len(best_set) >= int(getattr(_cfg, "PHASE3_MIN_RULES", 1))
                and round_best_score <= best_score + min_improve
            ):
                logger.info(
                    "Phase 3 greedy early-stop at round %d/%d: candidate_score=%.2f <= previous_score=%.2f",
                    k, max_rules, round_best_score, best_score,
                )
                break
