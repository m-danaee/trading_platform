...
from gpu_fuzzy_trader.scoring import robust_ratio_score
from gpu_fuzzy_trader.postprocess.symbol_specializer import specialize_rule_symbols
...
class TradingEnv:
    ...
    def __init__(
        ...
        self.n_state = self.n_features + self.n_rules + 2                          
...
def _candidate_score_robust(
    ...
    """Score TP/SL/capital using return/maxDD plus fold/monthly robustness."""
    score = robust_ratio_score(
        ...
        min_fold_trades=int(getattr(_cfg, "PHASE4_ROBUST_MIN_FOLD_TRADES", 5)),
        dd_floor=float(getattr(_cfg, "RETURN_DD_FLOOR", 1.0)),
        include_train_gap=True,
    )

    total_cap = sum(float(r.get("capital_pct", 0.0)) for r in candidate_rule_set)
    max_exp = float(val_metrics.get("max_total_open_exposure", 0.0))
    score -= max(0.0, total_cap - float(getattr(_cfg, "PHASE4_MAX_TOTAL_CAPITAL", 35.0))) * 0.10
    score -= max_exp * float(getattr(_cfg, "PHASE4_ROBUST_EXPOSURE_WEIGHT", 0.002))
    return float(score)


def _robust_grid_optimize(
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    rule_set: dict,
    direction: str,
    ...
    """Randomized grid search scored on internal validation and purged folds."""
    rules = rule_set.get("rules_set", [])
    if not rules:
        return [], [], 0
    ...
    try:
        ...
        logger.warning("Robust grid [%s]: purged folds unavailable: %s", direction, exc)
        evaluate_rule_set_on_fold_engines = None                            
        summarize_fold_metrics = None                            

    combined_for_monthly = pd.concat([train_df, val_df], ignore_index=True)
    monthly_feature_names = [
        c for c in combined_for_monthly.columns
        if c not in set(_cfg.LABEL_COLUMNS) | set(_cfg.META_COLUMNS) | set(_cfg.INTERNAL_COLUMNS)
        and not str(c).startswith("_")
    ]

    tp_grid = tuple(float(x) for x in getattr(_cfg, "PHASE4_TP_GRID", (2.0, 3.0)))
    sl_grid = tuple(float(x) for x in getattr(_cfg, "PHASE4_SL_GRID", (1.0, 1.5)))
    cap_grid = tuple(float(x) for x in getattr(_cfg, "PHASE4_CAPITAL_GRID", (3.0, 5.0, 7.5, 10.0)))
    n_trials = max(1, min(int(n_trials), int(getattr(_cfg, "PHASE4_ROBUST_MAX_TRIALS", n_trials))))

    def build_candidate(use_current: bool = False) -> list[dict]:
        candidate: list[dict] = []
        for r in rules:
            if use_current:
                tp = float(r.get("tp", _cfg.PHASE2_TP))
                sl = float(r.get("sl", _cfg.PHASE2_SL))
                cap = min(float(r.get("capital_pct", cap_grid[0])), max(cap_grid))
            else:
                tp = rng.choice(tp_grid)
                sl = rng.choice(sl_grid)
                cap = rng.choice(cap_grid)
            candidate.append({
        ...
        return repair_rule_set(candidate, direction=direction, min_rules=1, max_rules=len(candidate))

    best_score = -np.inf
    best_params: list[dict] = []
    scores: list[float] = []
    candidates_done = 0
    log_iv = iteration_log_interval(n_trials, target_logs=20)

    for trial_idx in range(n_trials):
        candidate = build_candidate(use_current=(trial_idx == 0))
        try:
            train_metrics = train_engine.simulate_rule_set(candidate)
            val_metrics = val_engine.simulate_rule_set(candidate)
            if fold_engines and evaluate_rule_set_on_fold_engines is not None:
                fold_summary = evaluate_rule_set_on_fold_engines(candidate, fold_engines)
            elif summarize_fold_metrics is not None:
                fold_summary = summarize_fold_metrics([val_metrics])
            else:
                class _S:
                    worst_return_pct = float(val_metrics.get("total_return_pct", 0.0))
                    worst_profit_factor = float(val_metrics.get("profit_factor", 0.0))
                    worst_drawdown_pct = float(val_metrics.get("max_drawdown_pct", 0.0))
                    min_trades = int(val_metrics.get("executed_trades", 0))
                fold_summary = _S()
            monthly_summary = None
            if getattr(_cfg, "MONTHLY_VALIDATION_ENABLED", False):
                monthly_summary, _ = evaluate_rule_set_monthly(
                    combined_for_monthly, candidate, direction, feature_names=monthly_feature_names
                )
            score = _candidate_score_robust(train_metrics, val_metrics, fold_summary, candidate, monthly_summary=monthly_summary)
        except Exception as exc:
            logger.debug("Robust grid [%s] trial %d failed: %s", direction, trial_idx, exc)
            score = -1e9
        ...
        if should_log_step(trial_idx, n_trials, log_iv):
            logger.info(
                "Robust grid [%s]: trial %d/%d best_score=%.4f",
                direction, trial_idx + 1, n_trials, best_score,
            )

    best_idx = int(np.argmax(scores)) if scores else 0
    logger.info(
        "Robust grid [%s]: %d trials complete, best_idx=%d best_score=%.4f",
        direction, candidates_done, best_idx, best_score,
    )
    if not best_params or best_score <= -1e8:
        logger.warning(
            "Robust grid [%s]: no usable candidate found; keeping current Phase 3 parameters.",
            direction,
        )
        best_params = build_candidate(use_current=True)
        best_idx = 0
        if not scores:
            scores = [float(best_score if best_score > -np.inf else -1e9)]
...
class RL_Agent:
    ...
    def train(self) -> dict:
        ...
        if getattr(_cfg, "SYMBOL_SPECIALIZATION_ENABLED", True):
            try:
                specialized_rules = specialize_rule_symbols(
                    self.train_df, self.val_df, self.rule_set.get("rules_set", []), self.direction
                )
                self.rule_set = {**self.rule_set, "rules_set": specialized_rules}
                logger.info(
                    "Phase 4 [%s]: applied evaluator_v5 symbol specialization to %d rules.",
                    self.direction, len(specialized_rules)
                )
            except Exception as exc:
                logger.warning("Phase 4 [%s]: symbol specialization skipped: %s", self.direction, exc)

        optimizer_name = str(getattr(_cfg, "PHASE4_OPTIMIZER", "robust_grid")).lower()
        if optimizer_name == "robust_grid":
            logger.info(
                "Phase 4 [%s]: using robust_grid optimizer with purged-fold scoring.",
                self.direction,
            )
            n_trials = min(
                int(getattr(_cfg, "PHASE4_ROBUST_MAX_TRIALS", 1200)),
                _phase4_sample_count(self.total_timesteps, self.elbow_window),
            )
            optimized_params, val_returns, elbow_idx = _robust_grid_optimize(
        ...
        elif _SB3_AVAILABLE and _GYM_AVAILABLE:
        ...
        optimized_params = repair_rule_set(
            optimized_params,
            direction=self.direction,
            min_rules=1,
            max_rules=len(optimized_params) if optimized_params else int(getattr(_cfg, "PHASE3_MAX_RULES", 5)),
        )
