...
def _normalise_directions(direction: str | list[str] | tuple[str, ...] | None) -> tuple[str, ...]:
    """Return requested strategy directions as a tuple of long/short."""
    ...
    for value in values:
        if value not in ("long", "short"):
            raise ValueError(f"direction must be long, short, or both; got {value!r}")
        if value not in out:
            out.append(value)
    return tuple(out)
...
class Pipeline_Orchestrator:
    ...
    def __init__(
        ...
        direction: str | list[str] | tuple[str, ...] | None = None,
        run_final_test: bool = True,
    ) -> None:
        self._output_dir = _resolve_output_root(output_dir)
        self._directions = _normalise_directions(direction)
        self._run_final_test = bool(run_final_test)
    ...
    def run(self, force: bool = False) -> dict:
        ...
        with _temporary_output_paths(self._output_dir):
            ...
            try:
                ...
                if self._run_final_test:
                    phase5_result = self._run_phase5()
                else:
                    logger.info("Skipping Phase 5 evaluation for RB evaluator_v5 scoring.")
                    phase5_result = {}
    ...
    def _load_phase2_outputs(self) -> dict[str, list[dict]]:
        ...
        if missing:
            raise FileNotFoundError(
                "Phase 3 requires Phase 2 pool outputs for the requested directions. "
                f"Missing or invalid: {', '.join(missing)}"
            )
    ...
    def _load_phase3_outputs(self) -> dict[str, dict]:
        ...
        if missing:
            raise FileNotFoundError(
                "Phase 4 requires Phase 3 rule-set outputs for the requested directions. "
                f"Missing or invalid: {', '.join(missing)}"
            )
    ...
    def _ensure_phase5_inputs(self) -> None:
        ...
        if missing:
            raise FileNotFoundError(
                "Phase 5 requires Phase 4 optimized strategies for the requested directions. "
                f"Missing or invalid: {', '.join(missing)}"
            )
    ...
    def _run_phase3(
        ...
        if not force:
            ...
            if existing is not None and all(d in existing for d in self._directions):
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        default="both",
        help="Run both directions or only one direction. Useful for auto-search.",
    )
    parser.add_argument(
        "--skip-final-test",
        action="store_true",
        help="Skip the final evaluation phase.",
    )
